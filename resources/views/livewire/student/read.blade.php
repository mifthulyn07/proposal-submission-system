        ...
        <div class="bg-white overflow-hidden rounded-lg shadow rounded-lg">
            ...
            <div class="m-4">
                ...
                <div class="mb-4 text-lg font-semibold text-left text-gray-900 bg-white dark:text-white dark:bg-gray-800">
                    <h5 class="text-lg font-bold text-gray-900 dark:text-white">List of Students</h5>
                                ...
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    <th scope="row" class="flex items-center px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        @if($student->user->avatar)
                                            <div class="inline-block w-10 h-10 overflow-hidden bg-gray-300 rounded-full">
                                                <img class="object-cover w-10 h-10" src="{{ asset('storage/avatars/'.$student->user->avatar) }}" alt="avatar"/>
                                            </div>
                                        @else                    
                                    ...
                                    @if (auth()->user()->hasRole('coordinator') || auth()->user()->hasRole('kaprodi'))
                                        <td class="px-6 py-4 space-x-1 whitespace-nowrap">
                                            ...
                                            <a data-tooltip-target="tooltip-delete" href="" wire:click="deleteIdStudent({{ $student->id }})" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-red-600 rounded-lg hover:bg-red-800 focus:ring-4 focus:ring-red-300 dark:focus:ring-red-900" x-data="" x-on:click.prevent="$dispatch('open-modal', 'confirm-student-deletion')">
            ...
            {{-- delete modal --}}
            <x-modal wire:ignore.self name="confirm-student-deletion" :show="$errors->studentDeletion->isNotEmpty()" focusable>
                <div class="p-6">
                    <div class="p-2 text-center">
                        <svg aria-hidden="true" class="mx-auto mb-4 text-gray-400 w-14 h-14 dark:text-gray-200" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                        <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Are you sure you want to delete <span class="text-gray-900 font-semibold">{{ $deleteIdStudentName }}</span> from student role?</h3>
                    </div>
                    ...
                    <div class="flex justify-end">
                        ...
                        <x-danger-button class="ml-3" wire:click.prevent="deleteStudent()" x-on:click="$dispatch('close')">
