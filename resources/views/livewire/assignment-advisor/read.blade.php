                    ...
                    <div class="my-4">
                        <ol class="mt-3 divide-y divider-gray-200 dark:divide-gray-700">
                            @foreach($proposals as $proposal)
                                <li>
                                    <a href="#" wire:click="checkProposal({{ $proposal->id }})" wire:click.prevent class="items-center block px-1 py-3 sm:flex hover:bg-gray-100 dark:hover:bg-gray-700">
                                        @if($proposal->student->user->avatar)
                                            <img class="mr-3 object-cover w-10 h-10 rounded-full" src="{{ asset('storage/avatars/'.$proposal->student->user->avatar) }}" alt="avatar"/>
                                        @else                    
                                            <img class="mr-3" src="https://ui-avatars.com/api/?name={{ urlencode($proposal->student->user->name) }}&background=e6f0ff&rounded=true" alt="avatar" width="40">
                                        @endif
                                        <div class="text-gray-600 dark:text-gray-400">
                                            <div class="md:flex md:justify-center md:items-center">
                                                <div class="mr-2 text-base font-normal">
                                                    <span class="font-medium text-gray-900 dark:text-white">{{ $proposal->student->user->name }}</span>
                                                </div>
                                            ...
                                            <span class="inline-flex items-center text-xs font-normal text-blue-500 dark:text-blue-400">
                                                ...
                                                {{ Illuminate\Support\Carbon::parse($proposal->updated_at)->diffForHumans() }}
                                            </span> 
                                        </div>
                                    </a>
                                </li>
